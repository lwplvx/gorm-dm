 ...
 func (G2DB g2db) fromString(val string, param parameter, conn *DmConnection) (interface{}, error) {
	 ...
	 case DATETIME, DATETIME2:
		 if conn.FormatTimestamp != "" {
			 ...
			 // 仅当字符串是严格 RFC3339（如 "2026-03-12T14:39:12+08:00"）时，
			 // 回退到通用字符串解析（encodeByString 内部兼容 RFC3339）；
			 // 其它格式保持原报错行为，避免宽松解析产生静默数据错误。
			 if _, perr := time.Parse(time.RFC3339Nano, val); perr == nil {
				 return encodeByString(val, param.column, *conn)
			 }
			 return nil, err
		 }
