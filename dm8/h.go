 ...
 */
 ...
 func toDTFromTime(x time.Time) []int {
	 ...
	 ts[OFFSET_NANOSECOND] = x.Nanosecond()
	 _, tz := x.Zone()
 ...
 func toDTFromString(s string, dt []int) (dtype int, err error) {
	 ...
	 // 兼容 ISO8601/RFC3339 格式（如 "2026-03-12T14:39:12+08:00"、"2026-03-12T14:39:12Z"）。
	 // 达梦原生仅接受空格分隔的 "YYYY-MM-DD HH:MM:SS[.fffffffff] [+HH:MM]"，
	 // 对带 'T' 分隔符与时区偏移的 RFC3339 字符串无法解析（报 -6015）。
	 // 这里优先用 Go 标准库解析，成功则直接构造 dt，失败再走达梦原生解析逻辑。
	 if t, perr := time.Parse(time.RFC3339Nano, s); perr == nil {
		 _, offset := t.Zone()
		 dt[OFFSET_YEAR] = t.Year()
		 dt[OFFSET_MONTH] = int(t.Month())
		 dt[OFFSET_DAY] = t.Day()
		 dt[OFFSET_HOUR] = t.Hour()
		 dt[OFFSET_MINUTE] = t.Minute()
		 dt[OFFSET_SECOND] = t.Second()
		 dt[OFFSET_NANOSECOND] = t.Nanosecond()
		 dt[OFFSET_TIMEZONE] = offset / 60
		 return DATETIME_TZ, nil
	 }
